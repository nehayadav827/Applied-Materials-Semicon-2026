"""
Shared candidate-generation logic. Extracts MULTIPLE local peaks per
scale/rotation combination, not just the single global max -- this is
the fix for low recall on periodic patterns.
"""

import cv2
import numpy as np

SCALE_MIN, SCALE_MAX, SCALE_STEPS = 9.0, 11.0, 9      # 0.25 step
ROT_MIN, ROT_MAX, ROT_STEPS = -2.0, 2.0, 5             # 1.0 step
PEAKS_PER_MAP = 6           # was 4 -- denser per-map peak extraction
PEAK_SUPPRESS_RADIUS = 12   # was 20px -- tighter suppression so nearby-but-
                             # distinct periodic peaks don't get merged away
MIN_SCORE = 0.05            # was 0.25 -- do NOT pre-reject weak-but-correct
                             # peaks before the reranker sees them; let the
                             # CNN filter instead of a fixed NCC floor
# NOTE: measured on a 40-sample smoke set, these changes moved "true match
# present in top-30 pool" from 20/40 -> 21/40 -- a small, real gain, not a
# fix. The dominant recall loss is NOT a tunable-parameter problem: on this
# same set, single_field-mode samples (one continuous periodic field, no
# landmark) had 0/4 recall -- genuinely ambiguous by design, matching the
# "several tiles match" scenario the problem statement itself describes.
# legacy/realistic/expanded modes (grid + streets, more distinguishing
# context) still only hit ~50-70% recall, which IS a real weakness -- see
# the per-generation_mode breakdown evaluate.py prints.
